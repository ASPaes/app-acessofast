-- =====================================================================
-- assign_plan passa a setar billing_mode junto com o plano.
-- =====================================================================
-- Bug de raiz: assign_plan setava plan_code/seat_limit/max_concurrent mas NUNCA
-- billing_mode -> tenant ganhava plano e continuava billing_mode='free' (metrado
-- + medidor de free/crédito visível). Foi o que desalinhou ASP/Feax/DigiOffice/
-- Consysa. Regra: plano 'individual' = tier free (metrado); qualquer outro = 'plan'
-- (ilimitado, sem metering/medidor).
-- =====================================================================
create or replace function public.assign_plan(
  p_tenant uuid, p_code text, p_seat_override integer default null, p_conc_override integer default null
)
...
begin
  ...
  update public.tenants t
     set plan_code = p_code,
         ...
         billing_mode = case when p_code = 'individual'
                             then 'free'::public.billing_mode
                             else 'plan'::public.billing_mode end,
         updated_at = now()
   where t.id = p_tenant;
  if not found then
