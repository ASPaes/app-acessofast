-- =====================================================================
-- Billing — super_admin = ACESSO GERAL A TUDO (bypass total de medição).
-- =====================================================================
-- Antes: super_admin já era isento do gate de concorrência (plano) e do
-- bloqueio por trial/dunning, MAS ainda passava pela medição free/crédito.
-- Efeito colateral: ao conectar num device de um tenant metrado esgotado
-- (ex.: canary movido p/ o tenant 'teste' sem free/crédito), o super_admin
-- levava `no_credits` e não conseguia acessar.
--
-- Decisão (2026-07-27): super_admin conecta em QUALQUER device sem medição,
-- sem gate e sem cobrança. Registra o atendimento como NÃO-cobrado (source
-- 'plan', sem hard cap) apenas para rastro/visibilidade. Reconexão já era
-- grátis; aqui cobrimos também o PRIMEIRO acesso de um super_admin.
--
-- Única mudança vs. 20260726140000: novo ramo `v_role = 'super_admin'` no
-- topo da decisão de NOVO atendimento. Assinatura, colunas e grants iguais.
-- =====================================================================
...
begin
  ...
  if v_role = 'super_admin'::public.user_role then
    -- Super admin = ACESSO GERAL: sem gate de concorrencia, sem medicao, sem
    -- cobranca. Registra como nao-cobrado (source 'plan', sem hard cap) so p/ rastro.
...
-- ---------------------------------------------------------------------
-- billing_eligibility — MESMO bypass p/ super_admin.
-- O connect-device corta no `blocked_reason` ANTES de chamar create_access_grant
-- (etapa 1). Sem este curto-circuito, o super_admin levaria `no_credits` aqui e
-- nem chegaria ao grant. Para super_admin: sem escolha, sem bloqueio -> conecta.
-- Única mudança vs. a versão do B1: lookup de v_role + short-circuit no super.
-- ---------------------------------------------------------------------
...
begin
  ...
  -- Super admin = ACESSO GERAL: sem escolha, sem bloqueio. (Grant nao cobra.)
  if v_role = 'super_admin'::public.user_role then
