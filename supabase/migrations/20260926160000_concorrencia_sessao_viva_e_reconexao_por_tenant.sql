-- Concorrencia por SESSAO VIVA + reconexao presa ao tenant.
--
-- DOIS BUGS observados no teste de 26/09:
--
-- 1) A vaga de simultaneidade era ocupada por um GRANT do painel que nunca virou
--    sessao. create_access_grant insere connection_logs 'active' no clique
--    (antes de qualquer conexao real). O gate de concorrencia contava esse log,
--    entao um clique em Conectar que nao conecta segurava a unica vaga por ate
--    5 min e impedia conectar em outra maquina. O tecnico "nao esta conectado"
--    mas mesmo assim nao consegue abrir outra sessao.
--
--    FIX: a contagem passa a considerar so SESSAO VIVA:
--      coalesce(last_heartbeat_at, session_start) > now() - interval '2 minutes'
--    - sessao real bate heartbeat a cada 20s -> conta;
--    - grant recem-criado (< 2min, ainda no aperto de mao do Conectar) -> conta,
--      para dois cliques simultaneos nao furarem o teto;
--    - grant velho sem heartbeat (nunca conectou) -> NAO conta.
--    Fica mais permissivo de proposito: no maximo libera uma conexao a mais,
--    nunca bloqueia indevidamente.
--
-- 2) Uma sessao real nao cobrava. meter_external_session (e create_access_grant)
--    reusa "atendimento aberto" so por rustdesk_id, SEM conferir tenant. Quando
--    um device muda de dono com um atendimento ainda na janela (foi o caso do
--    208146940, movido da ASP para o tenant teste), a sessao no novo tenant
--    reusa o atendimento do tenant ANTIGO -> nao cobra ninguem.
--
--    FIX: o lookup de reconexao passa a exigir a.tenant_id = v_tenant. Reconexao
--    e sempre dentro do mesmo dono; atendimento de outro tenant nunca serve.
--
-- Assinaturas inalteradas -> CREATE OR REPLACE nas tres funcoes.
...
begin
  ...
  -- Sessao VIVA (ver cabecalho): heartbeat recente OU grant no aperto de mao.
  ...
  -- Reconexao: mesmo dono, mesma maquina, janela aberta.
  select exists(
...
begin
  ...
  -- Sessao VIVA (ver cabecalho da migration): grant sem heartbeat > 2min nao conta.
  ...
  -- Reconexao: mesmo dono (tenant), mesma maquina, janela aberta.
  select * into v_atend
...
begin
  ...
  -- Sessao VIVA (ver cabecalho da migration).
  ...
  select * into v_atend
    from public.atendimentos a
   where a.rustdesk_id = p_rustdesk_id and a.tenant_id = v_tenant
     and a.ended_at is null and a.window_expires_at > now()
   order by a.started_at desc limit 1;
  if found then
